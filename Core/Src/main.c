...
int main(void)
{
    ...
    {
        /* Lectura de sensores */
        float temperature = bmp280_read_temp(&bmp);            /* °C */
        float pressure    = bmp280_read_pres(&bmp) / 100.0f;   /* hPa */
        float altitude    = bmp280_read_alt (&bmp, 1013.25f);  /* m  */

        /* Formatea y envía por UART */
        char buffer[100];
        int len = snprintf(buffer, sizeof(buffer),
                           "Temp: %.2f C | Presion: %.2f hPa | Altitud: %.2f m\r\n",
                           temperature, pressure, altitude);
        HAL_UART_Transmit(&huart1, (uint8_t*)buffer, len, HAL_MAX_DELAY);

        HAL_Delay(1000);   /* 1 s entre mediciones */
    }
}
